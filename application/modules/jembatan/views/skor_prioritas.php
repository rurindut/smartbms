        ...
        <div class="card-body">
            <table class="table table-bordered" id="detail_jembatan">
                ...
                <tr>
                    ...
                    <th scope="col" rowspan="2">Nama Jembatan</th>
                    <th scope="col" rowspan="2">Propinsi</th>
                    <th scope="col" rowspan="2">Kabupaten</th>
                    <th scope="col" rowspan="2">Skor Prioritas</th>
                    <th scope="col" rowspan="2">Penanganan</th>
                    <th scope="col" rowspan="2">Biaya Penggantian</th>
                    <th scope="col" rowspan="2">Biaya Rehabilitasi</th>
                    <th scope="col" colspan="5">Perkiraan Kuantitas Kerusakan</th>
                    <th scope="col" colspan="5">Biaya Pemeliharaan</th>
                </tr>
                <tr>
                ...
                foreach($jembatan as $key => $val){
                    ...
                    <tr>
                        ...
                        <td>'.$val['propinsi'].'</td>
                        <td>'.$val['kabupaten'].'</td>
                        <td>'.$val['skor_total'].'</td>
                        <td>'.$val['kategori_penanganan'].'</td>
                    ';
                    if($val['id_penanganan'] == '2') {
                        echo '<td>&nbsp;</td><td>&nbsp;</td>';
                        foreach($val['kerusakan'][0] as $data) {
                            echo '<td>'.number_format($data,3).'</td>';
                        }
                        foreach($val['kerusakan'][1] as $data) {
                            echo '<td>Rp '.number_format($data,2,',','.').'</td>';
                        }
                    } else {
                        if($val['id_penanganan'] == '3') {
                            echo '<td>&nbsp;</td><td>Rp '.number_format($val['rehab'],2,',','.').'</td>';
                        } else if($val['id_penanganan'] > '3') {
                            echo '<td>Rp '.number_format($val['replace'],2,',','.').'</td><td>&nbsp;</td>';
                        } else {
                            echo '<td>&nbsp;</td><td>&nbsp;</td>';
                        }
                        for($j=0; $j<10; $j++) {
